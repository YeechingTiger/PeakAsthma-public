class SurveyTwilioReminderNotificationJob < ApplicationJob
    ...
    def calc_twilio_reminder
      ...
      for p in patients
        if p.user.disabled == false
          @patient_reward_record = PatientRewardRecord.where("patient_id=?", p.id).order(logging_over_day: :desc).first
