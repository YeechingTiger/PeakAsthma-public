class PatientVisitJob < ApplicationJob
    ...
    def perform
        ...
        for p in patients
            patient_visits_record = PatientVisitsRecord.where("patient_id = ?", p.id).first

            if patient_visits_record == nil
              patient_visits_record = PatientVisitsRecord.new
              patient_visits_record.patient_id = p.id
